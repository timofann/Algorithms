...
template <typename T>
std::vector<int> GraphAlgorithms::search(const Graph &graph, int start_vertex) {
  ...
  start_vertex = GraphAlgorithms::validateVertex(graph, start_vertex);
  next_vertex_container.push(start_vertex);
  is_already_added[start_vertex] = true;
...
std::vector<int> GraphAlgorithms::breadthFirstSearch(const Graph &graph,
                                                     int start_vertex) {
  return search<queue<int>>(graph, start_vertex);
}

std::vector<int> GraphAlgorithms::depthFirstSearch(const Graph &graph,
                                                   int start_vertex) {
  return search<stack<int>>(graph, start_vertex);
}
