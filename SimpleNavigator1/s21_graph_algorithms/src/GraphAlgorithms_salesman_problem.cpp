...
static void
runAnts(const Graph &graph, std::vector<Ant> &ants,
        std::vector<std::vector<double>> &pheromone_lvl,
        int (*choosePathFunc)(const Graph &,
                              const std::vector<std::vector<double>> &,
                              const std::vector<int> &, const Ant &)) {
  ...
  for (auto ant = ants.begin(); ant != ants.end(); ant++) {
    ...
    while (available_paths.size() > 0) {
      path = choosePathFunc(graph, pheromone_lvl, available_paths, *ant);
...
GraphAlgorithms::solveTravelingSalesmanProblem(const Graph &graph) {
  ...
  for (int iter = 0; iter < ANTS_NUM / 2; ++iter) {
    ...
    runAnts(graph, ants, pheromone_lvl, &choosePath);
    findBestAnt(graph, ants, &best_ant);
    runAnts(graph, ants, pheromone_lvl, &chooseElitePath);
    findBestAnt(graph, ants, &best_ant);
  }
